...
class HDF5Loader():
    def __init__(self,
        ...
        if shuffle not in ("random", "seq", "none"):
            raise TypeError(r'shuffle mode has to be either ("random", "seq", "none")')

    def __getitem__(self, index):
        if self.shuffle == "random":
            indices = torch.randint(0, self.length - 1, (self.batch_size,))
        elif self.shuffle == "seq":
            random_index = torch.randint(0, self.length - 1 - self.batch_size, (1,))[0]
            indices = torch.arange(random_index, random_index + self.batch_size)
        elif self.shuffle == "none":
            if index + self.batch_size >= self.length:
                index = torch.randint(0, self.length - 1 - self.batch_size, (1,))[0]
            indices = torch.arange(index, index + self.batch_size)

        data = []
        for ind in indices:
            data.append(self.dataset[ind])

        coords, feats, labels = self.collate_fn(data)
